...
//    if (db.m_voc!=0) setVocabulary(*db.m_voc);  // original
    if (db.m_voc!=0) setVocabulary_change(*db.m_voc);  // change here
  }
...
void Database::setVocabulary_change(const Vocabulary& voc)
{
  delete m_voc;
        ...
        const FeatureVector &fv)
{
  ...
  EntryId update_entry_id = update_id;
  ...
  {
    //    std::find( m_ifile[i].begin(), m_ifile[i].end(), xx );
    m_ifile[i].remove_if( updateDBFromId( update_entry_id ) );
  }
  ...
  for(vit = v.begin(); vit != v.end(); ++vit)
  {
    ...
    ifrow.push_back(IFPair(update_entry_id, word_weight));
  }
  std::cout << "dbow update databse..ok " << std::endl;
}
