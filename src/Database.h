...
public:
  ...
  inline void setVocabulary(const Vocabulary &voc);
      ...
      // TODO change here => add 函数
    inline void setVocabulary_change(const Vocabulary &voc);
  ...
  inline const Vocabulary* getVocabulary() const;
    ...
    // TODO change here => add 函数
    const Vocabulary* getVoc_change() const;
  ...
  void allocate(int nd = 0, int ni = 0);
	...
	/**
	 * @brief invert direct
	 * update list  -->  change here
	 * @param features
	 * @param update_id  the if of update
	 * @param bowvec
	 * @param fvec
	 */
     void updateDb(const cv::Mat &features, EntryId update_id);

     void updateDb(const std::vector<cv::Mat> &features, EntryId update_id,
                   BowVector *bowvec = NULL, FeatureVector *fvec = NULL);

     void updateDb(const BowVector &vec, EntryId update_id,
  ...
  // TODO change here
  unsigned int set_m_nentries( unsigned int i )  { m_nentries = i; }
...
class updateDBFromId:public Database
{
public:
	unsigned int m_iID;

	bool operator( )( IFPair &mupdateId )
	{
		return mupdateId.entry_id == m_iID;
	}

	updateDBFromId(unsigned int iID)
	{
		m_iID = iID;
	}
